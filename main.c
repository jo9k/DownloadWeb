...
#define TIMESIZE 20	    /**<Length of timestamp*/
#define DEPTH 2		    /**<Maximum recurency depth*/
#define MAX_REQ 50		/**<Maximum number of requests from one page*/
...
char* getTimeBuffer(char *timebuff /**< [in/out] pointer to char string with timestamp*/);
 ...
 */
void die (char *s /**< [in] error message */, int i/**< [in] error number */)
{
	perror(s);
	exit(i);
}
...
/** List with new requests is filled with request found on the site. max number of requests is MAX_REQ
            \param website pointer to WebAddr struct
			\param log pointer to logfile
            \param content pointer to data to edit
			\param *new_requests pointer to list of new requests for further iterations
            \return 0 on success 1 on failure
 */
int editAndSave (WebAddr *website /**< [in,out] pointer to WebAddr struct */, FILE *log /**< [in,out] pointer to logfile */, char *content /**< [in,out] pointer to formatted data */, list *new_requests /**< [out] pointer to list of new http queries */);
{
